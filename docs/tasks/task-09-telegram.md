...
| `/myid` | Show the caller's Telegram chat ID |

## Data the Bot Gets from the Backend

### `/status` — calls `GET /api/cluster/nodes`
...
### `/latest` — calls `GET /api/detections/latest`
...
### Automatic Threat Alert (sent every 30 seconds if a new threat is found)
...
- Call `GET /api/alerts`
- For each alert where `acknowledged = false` → send a Telegram message
- Keep track of which alert IDs have already been sent (avoid duplicates)

### Sending Images

- Get `image_url` from detection data
- Download the image from the URL
- Send it as a photo to the Telegram chat
...
Must be connected to the cluster network (`192.168.50.x`).
